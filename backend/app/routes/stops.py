from fastapi import APIRouter, Depends, status
from sqlalchemy.ext.asyncio import AsyncSession

from app.controllers.itinerary_controller import add_itinerary_item
from app.controllers.stop_controller import delete_stop, update_stop
from app.database import get_db
from app.middleware.auth import get_current_user
from app.models.user import User
from app.schemas.common import APIResponse
from app.schemas.itinerary_item import (
    ItineraryItemCreate,
    ItineraryItemOut,
    StopActivityAssign,
    StopActivityOut,
)
from app.schemas.stop import StopOut, StopUpdate

router = APIRouter(prefix="/stops", tags=["Stops & Stop Activities"])


@router.post(
    "/{stop_id}/items",
    response_model=APIResponse[ItineraryItemOut],
    status_code=status.HTTP_201_CREATED,
    summary="Assign activity to a stop with date and time",
)
async def create_stop_itinerary_item(
    stop_id: str,
    payload: ItineraryItemCreate,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Assigns an activity to a stop with scheduled date, time, and custom cost."""
    item = await add_itinerary_item(db=db, stop_id=stop_id, current_user=current_user, payload=payload)
    return APIResponse(
        success=True,
        data=item,
        message="Itinerary item added to stop successfully",
    )


@router.post(
    "/{id}/activities",
    response_model=APIResponse[StopActivityOut],
    status_code=status.HTTP_201_CREATED,
    summary="Assign activity to a stop (alias)",
)
async def schedule_activity_to_stop(
    id: str,
    payload: StopActivityAssign,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Backwards compatible alias for /api/stops/{id}/items."""
    item = await add_itinerary_item(db=db, stop_id=id, current_user=current_user, payload=payload)
    return APIResponse(
        success=True,
        data=item,
        message="Activity scheduled to stop successfully",
    )


@router.put(
    "/{id}",
    response_model=APIResponse[StopOut],
    status_code=status.HTTP_200_OK,
    summary="Edit stop dates, notes, or order",
)
async def edit_stop(
    id: str,
    payload: StopUpdate,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Updates arrival/departure dates, notes, or changes the order sequence for a stop."""
    stop = await update_stop(db=db, stop_id=id, current_user=current_user, payload=payload)
    return APIResponse(
        success=True,
        data=stop,
        message="Stop updated successfully",
    )


@router.delete(
    "/{id}",
    response_model=APIResponse[dict],
    status_code=status.HTTP_200_OK,
    summary="Remove a stop from trip",
)
async def remove_stop(
    id: str,
    current_user: User = Depends(get_current_user),
    db: AsyncSession = Depends(get_db),
):
    """Removes a stop and its assigned activities from its parent trip."""
    result = await delete_stop(db=db, stop_id=id, current_user=current_user)
    return APIResponse(
        success=True,
        data=result,
        message="Stop removed successfully",
    )
